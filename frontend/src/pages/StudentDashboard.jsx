...
export default function StudentDashboard() {
  ...
  const [consultantCall, setConsultantCall] = useState({
    date: "2025-12-20",
    time: "14:00",
    note: "",
    status: "pending"
  ...
  const repRemaining = repLimit != null ? Math.max(repLimit - activeRepCount, 0) : null;

  useEffect(() => {
    const loadConvs = async () => {
      const res = await api.get("/conversations", { params: { userId: user.id } });
  ...
  useEffect(() => {
    const loadPrefs = async () => {
      ...
      const prefs = res.data && Object.keys(res.data).length ? res.data : null;
    ...
    const loadRatings = async () => {
      ...
      setRatings({ consultant: res.data.consultant ?? null, representative: res.data.representative ?? null });
    };
    loadPrefs();
    loadRatings();
  ...
  const filters = { city: preferences?.city || "" };
  if (preferences) {
    filters.noiseTolerance = preferences.noiseTolerance;
    filters.safetyImportance = preferences.safetyImportance;
    filters.transportImportance = preferences.transportImportance;
    filters.neighborsPreference = preferences.neighborsPreference;
    filters.marketDistancePreference = preferences.marketDistancePreference;
    filters.transitDistancePreference = preferences.transitDistancePreference;
    filters.socialPreference = preferences.socialPreference;
    filters.elevatorNeeded = preferences.elevatorNeeded;
    filters.wifiNeeded = preferences.wifiNeeded;
    filters.washerNeeded = preferences.washerNeeded;
  }
  ...
  const finalizeConsultantAssign = () => {
  ...
  const handleRepresentativeRequest = async (payload) => {
    if (!payload?.triggeredByClick) return { ok: false, message: "Please click Send request to create it." };
    if (!payload.repPackageChoice) return { ok: false, message: "Please select a representative package." };
    const packageChoice = preferences?.repPackage || payload.repPackageChoice;
    const limit = packageChoice === "5" ? 5 : 3;
    if (activeRepCount >= limit) return { ok: false, message: "No representative matches left in your package." };
    ...
    const note = JSON.stringify(meta);
    ...
    try {
      ...
      const updatedPrefs = { ...(preferences || {}), repPackage: packageChoice };
      updatePrefs(updatedPrefs);
      setToast({ text: "Representative request created.", type: "success" });
      setTimeout(() => setToast(null), 4000);
      setPayment({ open: true, item: { title: "Representative package", amount: packageChoice === "5" ? 40 : 30 } });
      return { ok: true, message: "Representative request sent. Awaiting owner approval." };
    } catch (err) {
      return { ok: false, message: err.response?.data?.message || "Request could not be created." };
    }
  };
  ...
  const repId = preferences.repPackage ? "u3" : null;
...
const consultantStatusText =
  ...
  const conversationLabel = (conv) => {
    ...
    if (repId && conv.participants.includes(repId)) return "Representative (Alice Cooper)";
  ...
  return (
    <div className="main-content">
      ...
      <div className="card" style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12, alignItems: "center" }}>
        ...
        {hasRep && <button className={`btn ${view === "representative" ? "btn-primary" : "btn-ghost"}`} onClick={() => setView("representative")}>Representative</button>}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 8 }}>
                {criteriaItems.map((item) => (
                  <div key={item.label} style={{
                    ...
                    <div style={{ fontWeight: 600, marginTop: 4 }}>{formatCriteriaValue(item.value)}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="card" style={{ display: "grid", gap: 6 }}>
              ...
              {repAppointments.slice(-4).map((r) => (
                <div key={r.id} style={{ border: "1px solid var(--border)", borderRadius: 10, padding: "8px 10px", background: "#f8fafc" }}>
                  ...
                  <div style={{ color: "var(--muted)" }}>Status: {normalizeStatus(r.status)}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {view === "tours" && (
        <div className="grid-two" style={{ marginTop: 10 }}>
              ...
              <ul className="list-clean" style={{ color: "var(--muted)", lineHeight: 1.5 }}>
                ...
                {repAppointments.map((r) => (
                  <li key={r.id} style={{ borderBottom: "1px solid var(--border)", paddingBottom: 6 }}>
                    {r.meta.propertyTitle || "-"} - {r.meta.session || "-"} - {r.meta.language || "-"} - {normalizeStatus(r.status).toUpperCase()}
                  </li>
                ))}
              </ul>
            </div>
          ...
          <PropertyDetail
            ...
            onSelectRepPackage={(choice) => updatePrefs({ ...(preferences || {}), repPackage: choice })}
                ...
                <div style={{ display: "grid", gap: 8 }}>
                  <label>
                    ...
                    <select
                      ...
                      onChange={(e) => updatePrefs({ ...(preferences || {}), consultantFocus: e.target.value })}
                    >
                  ...
                  <label>
                    ...
                    <select
                      ...
                      onChange={(e) => updatePrefs({ ...(preferences || {}), consultantLanguages: e.target.value })}
                    >
                      <option value="">Select</option>
              ...
              {repAppointments.map((r) => {
                ...
                return (
                  <li key={r.id} style={{ border: "1px solid var(--border)", borderRadius: 10, padding: "8px 10px", background: "#f8fafc" }}>
                    <div style={{ fontWeight: 600 }}>{r.meta.propertyTitle || "-"}</div>
                    <div>{r.meta.session || "-"} • {r.meta.language || "-"}</div>
                    <div>Status: {normalizeStatus(r.status).toUpperCase()}</div>
      ...
      <PaymentModal
        open={payment.open}
        onClose={() => setPayment({ open: false, item: null })}
        item={payment.item}
        onConfirm={() => {
          if (payment.item?.type === "consultant") {
            finalizeConsultantAssign();
          }
        }}
      />
    </div>
